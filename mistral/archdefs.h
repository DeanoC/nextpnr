...
// rnode_coords{} (all zero) is what libmistral returns for "no node"
static const CycloneV::rnode_coords invalid_rnode{};

// libmistral's coordinate labels as hash keys
template <> struct hash_ops<CycloneV::rnode_coords>
{
    static inline bool cmp(const CycloneV::rnode_coords &a, const CycloneV::rnode_coords &b) { return a == b; }
    static inline unsigned int hash(const CycloneV::rnode_coords &a) { return a.v; }
};
template <> struct hash_ops<CycloneV::pnode_coords>
{
    static inline bool cmp(const CycloneV::pnode_coords &a, const CycloneV::pnode_coords &b) { return a == b; }
    static inline unsigned int hash(const CycloneV::pnode_coords &a) { return mkhash(unsigned(a.v), unsigned(a.v >> 32)); }
};
...
struct PipId
{
    ...
    // The coordinate labels convert to bool, so hash their raw values: with
    // mkhash(src, dst) every pip hashed alike and pip-keyed lookups took
    // 100 us each
    unsigned int hash() const { return mkhash(src.v, dst.v); }
};
