module top(input clka, clkb, wea, web, ena, enb,
 input [1:0] bea, beb, input [8:0] aa, ab, input [19:0] da, db, output [19:0] qa, qb);
 altsyncram #(.intended_device_family("Cyclone V"), .operation_mode("BIDIR_DUAL_PORT"),
 .ram_block_type("M10K"), .width_byteena_a(2), .width_byteena_b(2), .byte_size(10),
 .byteena_reg_b("CLOCK1"), .width_a(20), .widthad_a(9), .numwords_a(512),
 ...
 .address_a(aa), .data_a(da), .q_a(qa), .wren_a(wea), .rden_a(1'b1), .byteena_a(bea),
 .address_b(ab), .data_b(db), .q_b(qb), .wren_b(web), .rden_b(1'b1), .byteena_b(beb),
