#!/usr/bin/env python3
"""Infer and route one byte-masked true dual-port M10K with two enabled read/write ports."""
import argparse
import copy
import json
...
def diagnostics(args, design, name, out):
    for label, params, disconnect, message in (
        ('width10', {'CFG_DBITS':10,'CFG_ABITS':10}, None, 'byte enables require 512x20'),
        ('address', {'CFG_ABITS':8}, None, 'true dual-port requires'),
        ('mixed', {'CFG_MIXED_WIDTH':1}, None, 'cannot combine'),
        ('mask-a', {}, 'A1BE', 'byte mode requires connected'),
        ('mask-b', {}, 'B1BE', 'byte mode requires connected'),
        ('mask-wide', {}, None, 'masks must have exactly two bits'),
        ...
        if label == 'mask-wide': cell['connections']['A1BE'].append('1')
    ...
    print('PASS: seven invalid byte TDP configurations rejected',flush=True)


def constant_masks(args, design, name, out):
    case=out/'constant-masks';case.mkdir(exist_ok=True)
    variant=copy.deepcopy(design)
    cell=variant['modules']['top']['cells'][name]
    cell['connections']['A1BE']=['1','0']
    cell['connections']['B1BE']=['0','1']
    fixture=case/'synth.json';fixture.write_text(json.dumps(variant))
    run([str(args.nextpnr.resolve()),'--device','5CSEBA6U23I7','--freq','50',
         '--qsf',str(args.qsf.resolve()),'--sdc',str(args.sdc.resolve()),'--json',str(fixture),
         '--compress-rbf','--rbf',str(case/'top.rbf'),'--write',str(case/'routed.json')],case/'route.log')
    ...
    for side in 'AB':
        assert len(packed['connections'][side+'1BE'])==2
        for bit in (0,1):
            assert re.search(r'^r \S+ '+re.escape(f'{site}:BYTEENABLE{side}.{bit}')+r'$',bt,re.M)
    print('PASS: complementary constant 0/1 masks retain all four routes',flush=True)
...
def main():
    ...
    for width,same,raw in ((20,0,0),(16,0,0),(20,1,0),(16,1,0),(20,0,1)):
        label=f'w{width}-c{same}' + ('-raw' if raw else '')
        ...
        script=case/'synth.ys';script.write_text(f'read_verilog {source}\n'
             f'chparam -set WIDTH {width} -set SAME_CLOCK {same} -set RAW {raw} top\n'
        ...
        assert int(cell['parameters']['CFG_BYTE_ENABLE'],2)==1
        assert all(len(cell['connections'][side+'1BE'])==2 for side in 'AB')
        if (width,same,raw)==(20,0,0):
            diagnostics(a,design,name,case)
            constant_masks(a,design,name,case)
        ...
        if not same and not raw:
            assert report['fmax']['clock_b']['constraint']==25
        if not same:
        ...
        for pin in ('CLKIN.0','CLKIN.1','WREN.0','WREN.1','ENABLE.0','ENABLE.1','BYTEENABLEA.0','BYTEENABLEA.1','BYTEENABLEB.0','BYTEENABLEB.1'):
            assert re.search(r'^r \S+ '+re.escape(site+':'+pin)+r'$',bt,re.M),pin
        print(f'PASS: {label}: one byte TDP M10K, four mask routes, clocks, selectors, RBF and 50 MHz',flush=True)
