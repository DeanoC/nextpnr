# Registered clock-enable status output
...
bit 0 controls a falling-edge `cyclonev_clkena` with power-up low. Its gated
...
Clock-enable settings remain `REG1_ENOUT`, power-up 0, INPUT_SEL `16` and
...
```sh
quartus_sh -t /path/to/fixtures/clock-enable-status/oracle.tcl
