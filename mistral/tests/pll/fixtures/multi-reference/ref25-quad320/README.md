# ref25-quad320 Quartus reference

Quartus Prime Lite 17.0.2, device `5CSEBA6U23I7`, 25 MHz reference,
40/80/16/20 MHz outputs and 25/75/25/75% duties, direct integer mode, zero phase.
...
zero phases in `fitter-pll.txt`. M64/N5, reported VCO 320 MHz.
Fixed output counter placements: C6 divide 8 (high 2/low 6), C7 divide 4 (high 3/low 1), C5 divide 20 (high 5/low 15), C8 divide 16 (high 12/low 4).
The checked feedback tuple uses BWCTRL 3, CP_CURRENT 2, M low preset 7,
and M phase preset 3. Full FPLL values, including defaults, must be compared
...
```sh
quartus_sh -t /path/to/fixtures/multi-reference/ref25-quad320/oracle.tcl
