# Fabric-controlled PLL clock enable, power-up low
...
The enable register starts low. This fixture does not establish hardware
...
`CMUXHG.000.035.2:ENABLE` via `TD.000.036.0099`. ENABLE_REGISTER_POWER_UP.2
is explicitly 0. Relative to the [power-up-high oracle](../clock-enable/README.md),
this is the only difference in all emitted FPLL/CMUX settings and the enable
endpoint route. The high oracle omits this setting because its default is 1.
...
```sh
quartus_sh -t /path/to/fixtures/clock-enable-low/oracle.tcl
